...
import { z } from 'zod';
import { db } from '../db';
import { metinBoss, mb_tmp, mappa_mbt, mappe, config } from '../db/schema';
...
router.get('/metin-boss/:id', (c) => {
  ...
  if (!mb) return c.json({ error: 'Metin/Boss non trovato' }, 404);
  return c.json(mb);
});
...
router.patch('/metin-boss/:id', zValidator('json', metinBossUpdateSchema), (c) => {
  ...
  if (!existing) return c.json({ error: 'Metin/Boss non trovato' }, 404);
...
router.get('/mb-tmp/:id', (c) => {
  ...
  if (!row) return c.json({ error: 'MbTmp non trovato' }, 404);
  return c.json(row);
});
...
router.patch('/mb-tmp/:id', zValidator('json', mbTmpUpdateSchema), (c) => {
  ...
  if (!existing) return c.json({ error: 'MbTmp non trovato' }, 404);
...
router.get('/mappa-mbt/:id', (c) => {
  ...
  if (!row) return c.json({ error: 'MappaMbt non trovata' }, 404);
  return c.json(row);
});
...
router.patch('/mappa-mbt/:id', zValidator('json', mappaMbtUpdateSchema), (c) => {
  ...
  if (!existing) return c.json({ error: 'MappaMbt non trovata' }, 404);
...
router.get('/mappe/:id', (c) => {
  ...
  if (!map) return c.json({ error: 'Mappa non trovata' }, 404);
  return c.json(map);
});
...
router.patch('/mappe/:id', zValidator('json', mappaSchema.partial()), (c) => {
  ...
  if (!existing) return c.json({ error: 'Mappa non trovata' }, 404);
