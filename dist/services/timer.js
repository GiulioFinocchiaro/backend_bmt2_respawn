...
import { config, mb_tmp, metinBoss, mappa_mbt, mappe } from '../db/schema.js';
import { eq, sql, or } from 'drizzle-orm';
function getDataRiavvio() {
...
export function getAllTimers() {
    ...
    console.log(all);
    return all.map(t => calculateTimer(t.id)).filter(Boolean);
}
export function getAllMetinTimers() {
    const allMetin = db.select().from(mb_tmp).where(eq(metinBoss.categoria, 'Metin')).all();
    return allMetin.map(t => calculateTimer(t.id)).filter(Boolean);
}
export function getAllBossTimers() {
    const allBoss = db.select().from(mb_tmp).where(eq(metinBoss.categoria, 'Boss')).all();
    return allBoss.map(t => calculateTimer(t.id)).filter(Boolean);
}
export function searchAllTimers(query) {
    const pattern = `%${query.toLowerCase()}%`;
    const all = db.select()
        .from(mb_tmp)
        .where(or(sql `LOWER(${mb_tmp.descrizione}) LIKE ${pattern}`, sql `LOWER(${metinBoss.nome}) LIKE ${pattern}`, sql `LOWER(${metinBoss.categoria}) LIKE ${pattern}`, sql `LOWER(${metinBoss.note}) LIKE ${pattern}`, sql `LOWER(${mappe.nome}) LIKE ${pattern}`))
        .all();
    return all.map(t => calculateTimer(t.id)).filter(Boolean);
}
